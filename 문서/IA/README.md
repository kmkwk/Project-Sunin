...
1. 회원가입  
    1)개인
  - ~~카카오톡으로 회원가입~~
  - 이메일로 회원가입 -> 이메일 입력 -> 이메일 중복검사 (중복있을시 input창 밑에 아이디 사용 불가능 알림)-> 이름 입력 -> 닉네임 입력 -> 닉네임 중복검사 (중복있을시 input창 밑에 닉네임 사용불가능 알림) -> 비밀번호 입력 -> 비밀번호 확인 -> 비밀번호 같은지 확인 (비밀번호 다를시 비밀번호 확인창 밑에 비밀번호가 다르다고 알림) -> 회원가입버튼 누르면 alert (가입완료) -> 로그인페이지로 이동
  - ~~기본 회원가입 -> 이름 입력 -> 닉네임 입력 -> 닉네임 중복검사 -> 비밀번호 입력 -> 비밀번호 확인 -> 비밀번호 같은지 확인 -> 핸드폰 번호 입력 -> 핸드폰 중복검사 및 본인 확인과정? -> 회원가입 후 로그인페이지로 이동~~
  - ~~SNS 페이스북이나 네이버 등등 아이디로 회원가입~~

    2)기업
  - 이메일로 회원가입 -> 이메일 입력 -> 이메일 중복검사 (중복있을시 input창 밑에 아이디 사용 불가능 알림)-> 이름 입력 -> 닉네임 입력 -> 닉네임 중복검사 (중복있을시 input창 밑에 닉네임 사용불가능 알림) -> 비밀번호 입력 -> 비밀번호 확인 -> 비밀번호 같은지 확인 (비밀번호 다를시 비밀번호 확인창 밑에 비밀번호가 다르다고 알림) -> 사업자 등록증 사진 첨부 -> 회원가입버튼 누르면 alert (가입완료) -> 로그인페이지로 이동


2. 썬인랭킹  
1)개인랭킹
  - 썬인많은순 ~~or 좋아요 많은순으로 검색가능~~ (기본은 썬인 많은순) -> 리스트로 띄움 -> 이름 클릭시 프로필로 이동 -> 팔로우 클릭시 로그인페이지로 이동  
    2)회사랭킹
  - 썬인많은순 ~~or 좋아요 많은순으로 검색가능~~ (기본 썬인 많은순) -> 리스트로 띄움 -> 이름 클릭시 프로필로 이동 -> 팔로우 클릭시 로그인페이지로 이동

3. 검색  
1)해시태그 검색  
  - 검색창 안에 (해시태그입력) 알려줌 -> 해시태그로 검색 -> 검색 해시태그 단어 포함함 피드 정렬 -> 1)인기순, ~~2)최신순 정렬 가능 (기본 인기순)~~-> 게시물 하트 클릭시 로그인페이지로 이동  
2)프로필 검색  
  - 검색창 안에 (아이디입력) 알려줌 -> 아이디로 검색 -> 검색 단어 포함한 아이디 정렬 -> 클릭시 프로필로 이동 -> 팔로우 클릭시 로그인페이지로 이동
...
* 회원정보 조회
  * [이메일], [비밀번호], [이름], [아이디], [닉네임]을 조회할 수 있음
...
* 기업 인증여부 조회(기업회원의 경우)
  * 기업회원의 경우, 기업인증이 필요.
    ...
    * 인증이 완료되었다면 인증완료 표시
    * 반려되었다면 인증실패 표시
        * 재인증 요청 필요 - 알림 & 관리자이메일로 요청해주세요

2. 회원정보 수정
* 수정버튼 클릭 시 회원정보 수정 가능

* 회원정보(프로필) 수정
    * 기존에 입력되어 있는 [이메일], [비밀번호], [이름], [아이디], [닉네임]이 조회된 상태.
    * 이메일 변경 불가
    * 닉네임, 비밀번호 변경 가능
        *닉네임 변경 시 중복 여부 확인
